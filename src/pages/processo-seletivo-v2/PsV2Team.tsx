...
import { AlertTriangle, ArrowLeft, ArrowRight, CalendarDays, Database, ExternalLink, FileText, MapPinned, Search, ShieldCheck, SlidersHorizontal, Users } from 'lucide-react';
...
export default function PsV2Team() {
  ...
  return <MainLayout>
    ...
    {isLoading ? <Card className="rounded-2xl border-dashed"><CardContent className="p-8 text-center text-sm text-muted-foreground">Carregando eventos...</CardContent></Card> : filteredEvents.length ? <div className="grid gap-3 lg:grid-cols-2">{filteredEvents.map((event: any) => {
      ...
      return <Card key={event.id} className="rounded-2xl border-border/60 bg-card/70 transition hover:border-primary/25 hover:shadow-md"><CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><CardTitle className="truncate text-base">{event.name}</CardTitle><CardDescription className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1"><span className="inline-flex items-center gap-1.5"><CalendarDays className="h-3.5 w-3.5" />{formatDate(event.date)}</span><span>{event.location || 'Local ainda não informado'}</span></CardDescription></div><Badge variant={event.status === 'em_andamento' ? 'default' : 'secondary'}>{PS_EVENT_STATUS[event.status] || event.status || 'Sem status'}</Badge></div></CardHeader><CardContent className="space-y-3 border-t border-border/50 pt-4"><div className="flex items-center gap-2 text-xs text-muted-foreground"><Users className="h-4 w-4 text-primary" />Fluxo V2 seguro completo até a barreira anterior à escrita oficial.</div><div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" onClick={() => setStaffingEventId(event.id)}><MapPinned className="mr-2 h-3.5 w-3.5" />Necessidades</Button><Button asChild variant="outline" size="sm"><Link to={`${eventBase}?view=preparacao`}><FileText className="mr-2 h-3.5 w-3.5" />Preparação</Link></Button><Button asChild variant="outline" size="sm"><Link to={eventBase}>Alocação</Link></Button><Button asChild variant="outline" size="sm"><Link to={`${eventBase}?view=prontidao`}><ShieldCheck className="mr-2 h-3.5 w-3.5" />Prontidão</Link></Button><Button asChild variant="outline" size="sm"><Link to={`${eventBase}?view=pendencias`}><AlertTriangle className="mr-2 h-3.5 w-3.5" />Pendências</Link></Button><Button asChild variant="outline" size="sm"><Link to={`${eventBase}?view=relatorios`}><FileText className="mr-2 h-3.5 w-3.5" />Relatórios</Link></Button><Button asChild variant="outline" size="sm"><Link to={`${eventBase}?view=integracao`}><Database className="mr-2 h-3.5 w-3.5" />Pré-integração</Link></Button><Button asChild size="sm"><Link to={`${eventBase}?view=overview`}>Central do evento <ArrowRight className="ml-2 h-4 w-4" /></Link></Button></div></CardContent></Card>;
