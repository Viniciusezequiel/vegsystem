...
const render=(template:string,values:Record<string,string>)=>['nome','evento','cargo','unidade','predio','andar','sala','horario','link_confirmacao'].reduce((text,key)=>text.replaceAll(`{{${key}}}`,values[key]||''),template);
...
serve(async req=>{
  ...
  try {
    ...
    const {data:event}=await admin.from('ps_events').select('id,name').eq('id',eventId).maybeSingle(); if(!event) return json({error:'event_not_found'},404);
    ...
    const result={total:jobs.length,sent:0,failed:0,missingRecipient:0,pending:Math.max(0,jobs.length-processingJobs.length),quotaWaiting:0,details:[] as Array<Record<string,unknown>>}; let testProcessed=0;
    for(const job of processingJobs){
      ...
      const {data:link}=await admin.from('ps_event_collaborators').select('id,event_id,collaborator_name,email,role_name,assigned_role,unit,building,floor,room,work_schedule,participation_status').eq('id',job.event_collaborator_id).eq('event_id',eventId).maybeSingle();
      ...
      const logical=String(link.email||'').trim(); if(!logical){await admin.from('ps_event_communications').update({status:'failed_missing_recipient',failed_at:new Date().toISOString(),last_error:'missing_recipient',updated_at:new Date().toISOString()}).eq('id',job.id);result.missingRecipient++;result.details.push({id:job.id,status:'failed_missing_recipient'});continue;}
      if(testMode&&testProcessed>=testBatchLimit){result.pending++;result.details.push({id:job.id,status:job.status});continue;}
      ...
      try{
        ...
        const text=render(job.body_template,{nome:link.collaborator_name,evento:event.name,cargo:link.role_name||link.assigned_role||'',unidade:link.unit||'',predio:link.building||'',andar:link.floor||'',sala:link.room||'',horario:link.work_schedule||'',link_confirmacao:confirmationUrl});
        const delivered=await provider.send({to:testMode?testRecipient:logical,subject:job.subject,text,html:`<div style="white-space:pre-wrap;font-family:Arial,sans-serif">${escapeHtml(text)}</div>`,metadata:{module:'process-selection',event_id:eventId,type:job.communication_type}});
    ...
    return json({mode:testMode?'test':'production',provider:provider.name,...result});
