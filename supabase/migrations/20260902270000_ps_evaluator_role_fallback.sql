...
BEGIN
  ...
  LOOP
    ...
    WHERE lower(r.value) IN (
            lower(trim(coalesce(v_link.assigned_role, ''))),
            lower(trim(coalesce(v_link.role_name, ''))),
            lower(trim(coalesce(v_link.role_value, '')))
          )
       OR lower(r.name) IN (
            lower(trim(coalesce(v_link.assigned_role, ''))),
            lower(trim(coalesce(v_link.role_name, ''))),
    ...
    v_role := lower(
      concat_ws(' ',
        nullif(trim(v_link.assigned_role), ''),
        nullif(trim(v_link.role_name), ''),
        nullif(trim(v_link.role_value), '')
      )
    );
