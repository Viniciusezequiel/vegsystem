CREATE OR REPLACE FUNCTION public.ps_mask_attendance_pix(p_value text)
RETURNS text
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT CASE
    WHEN nullif(trim(p_value), '') IS NULL THEN 'Não informado'
    WHEN position('@' in p_value) > 1 THEN
      left(split_part(p_value, '@', 1), 2) || '***@' || split_part(p_value, '@', 2)
    WHEN length(regexp_replace(p_value, '\s', '', 'g')) <= 4 THEN '••••'
...
REVOKE ALL ON FUNCTION public.ps_public_get_attendance_details(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.ps_public_confirm_attendance_details(uuid, boolean, text, boolean, text, text) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.ps_public_get_attendance_details(uuid)
TO anon, authenticated;

GRANT EXECUTE ON FUNCTION public.ps_public_confirm_attendance_details(uuid, boolean, text, boolean, text, text)
TO anon, authenticated;
